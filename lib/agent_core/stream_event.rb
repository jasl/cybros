...
module AgentCore
  ...
  #
  # Used by provider adapters (LLM streaming) and DAG executors to communicate
  # progress to callers. Callers subscribe via block/callback and receive these
  # events in real-time.
  #
  ...
  module StreamEvent
    ...
    #
    # Emitted by DAG executors when the tool policy returns Decision.confirm
    # for one or more tool calls.
    ...
    #
    # Emitted when a tool call must be executed outside the current streaming
    # loop (pause/resume semantics).
    ...
    # streaming response. DAG executors may also emit this once for the entire
    # "tool loop" (providers' per-call Done events are not forwarded).
    class Done
