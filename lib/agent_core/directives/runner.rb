    ...
    class Runner
      ...
      def run(
        ...
        schema_name =
          if structured_output_options.is_a?(Hash)
            structured_output_options.fetch(:schema_name, nil) || AgentCore::Directives::Schema::NAME
          else
            AgentCore::Directives::Schema::NAME
          end
        ...
        base_system =
          [
            system.to_s,
            AgentCore::Directives::ENVELOPE_OUTPUT_INSTRUCTIONS,
            output_instructions,
        ...
        @directives.fetch(:modes).each do |mode|
          ...
          next unless AgentCore::Directives::DEFAULT_MODES.include?(mode_sym)
          ...
          (1 + retry_budget).times do |attempt_index|
            ...
            llm_options =
              AgentCore::Utils.deep_merge_hashes(
            ...
            assistant_message, assistant_text =
              begin
                ...
                Array(history).each { |m| messages << m }
      ...
      def normalize_llm_options_hash(value)
        ...
        ValidationError.raise!(
          ...
          code: "agent_core.directives.runner.llm_options_defaults_must_be_a_hash",
      ...
      def normalize_directives_config(value)
        ...
        modes = Array(cfg.fetch(:modes, AgentCore::Directives::DEFAULT_MODES)).compact
        modes = modes.map { |m| m.to_s.strip.downcase.tr("-", "_").to_sym }
        modes = modes.select { |m| AgentCore::Directives::DEFAULT_MODES.include?(m) }
        modes = AgentCore::Directives::DEFAULT_MODES if modes.empty?

        repair_retry_count =
          Integer(
            cfg.fetch(:repair_retry_count, AgentCore::Directives::DEFAULT_REPAIR_RETRY_COUNT),
            exception: false,
          )
        if repair_retry_count.nil? || repair_retry_count < 0
          repair_retry_count = AgentCore::Directives::DEFAULT_REPAIR_RETRY_COUNT
        end
      ...
      def response_format_for(mode, schema_name:, allowed_types:)
        case mode
        when :json_schema
          AgentCore::Directives::Schema.response_format(
            strict: true,
            name: schema_name,
            types: allowed_types,
          )
      ...
      def parse_directives_envelope(assistant_message, assistant_text, allowed_types:, type_aliases:, payload_validator:, max_bytes:)
        ...
        parsed =
          AgentCore::Directives::Parser.parse_json(
            assistant_text,
        ...
        validated =
          AgentCore::Directives::Validator.validate(
            parsed[:value],
            allowed_types: allowed_types,
            type_aliases: type_aliases,
            payload_validator: payload_validator,
      ...
      def normalize_max_bytes(raw_value)
        return AgentCore::Directives::Parser::DEFAULT_MAX_BYTES if raw_value.nil?

        parsed = Integer(raw_value)
        return AgentCore::Directives::Parser::DEFAULT_MAX_BYTES if parsed <= 0

        parsed
      rescue ArgumentError, TypeError
        AgentCore::Directives::Parser::DEFAULT_MAX_BYTES
      end
      ...
      def validate_llm_options!(llm_options)
        ...
        ValidationError.raise!(
          ...
          code: "agent_core.directives.runner.directives_llm_options_contains_reserved_keys",
