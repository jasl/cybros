...
module AgentCore
  module Contrib
    ...
    #
    # This keeps provider-specific model IDs out of AgentCore core while still
    ...
    module TokenEstimation
      ...
      def estimator(root: default_root)
        ...
        ESTIMATOR_MUTEX.synchronize do
          @estimators ||= {}
          @estimators[cache_key] ||= TokenEstimator.new(registry: registry(root: root, tokenizer_root_path: root_dir))
        end
      end
      ...
      def default_root
        ...
        raise(
          ConfigurationError,
          "Token estimation root is not configured. Set AgentCore::Contrib::TokenEstimation.configure(root: ...) " \
