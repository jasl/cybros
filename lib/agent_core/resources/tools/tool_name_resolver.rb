...
module AgentCore
  module Resources
    module Tools
      ...
      # - Optional heuristic normalize fallback (disabled by default at runtime)
      module ToolNameResolver
        ...
        def resolve(requested_name, include_check:, aliases: {}, enable_normalize_fallback: false)
          ...
          if enable_normalize_fallback
            normalized = normalize_fallback(requested)
            if normalized != requested && include_check.call(normalized)
              return Resolution.new(requested_name: requested, resolved_name: normalized, method: :normalized)
            end
          end
