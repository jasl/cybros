...
require "tmpdir"
require "webrick"
require Rails.root.join("lib/cybros/agents/claw")
require Rails.root.join("test/support/callback_harness")
...
class RPCContractTest < ActiveSupport::TestCase
  ...
  test "application uses CLAW_WORKSPACE_ROOT as the default live workspace root" do
    Dir.mktmpdir("claw-env-workspace-") do |workspace_root|
      File.write(File.join(workspace_root, "AGENTS.md"), "Env AGENTS\n")

      with_env("CLAW_WORKSPACE_ROOT" => workspace_root) do
        env_application =
  ...
  test "before_agent_step falls back to a local tool surface manifest when callback RPC is unreachable" do
    capability_snapshot = {
      "capability_registry_snapshot_id" => "csnap_fixture",
      "effective_tools" => [
        {
      ...
      application.call(
        ...
        params: {
          ...
          "callback_session" => {
            ...
            "bearer" => "secret://callback"
          }
        }
      )

    expected_surface_id =
      "surface_#{Digest::SHA256.hexdigest(JSON.generate({
        capability_registry_snapshot_id: "csnap_fixture",
        selected_tool_ids: %w[etool_compact etool_subagent_spawn]
      })).first(24)}"

    assert_equal "csnap_fixture", payload.dig("planning", "tool_surface", "capability_registry_snapshot_id")
    assert_equal %w[etool_compact etool_subagent_spawn], payload.dig("planning", "tool_surface", "selected_tool_ids")
    assert_equal expected_surface_id, payload.dig("planning", "tool_surface", "tool_surface_id")
    assert_equal "bundled_claw.before_agent_step", payload.dig("planning", "tool_surface", "tool_surface_label")
    assert_equal %w[compact_context subagent_spawn], payload.dig("planning", "tool_surface", "logical_tool_names")
  end
  ...
  def with_skill_catalog_sources(sources)
    singleton = Cybros::Agents::Claw::RuntimeSettings.singleton_class
  ...
  def with_platform_skill_dirs(dirs)
    singleton = Cybros::Agents::Claw::RuntimeSettings.singleton_class
    original_method = singleton.instance_method(:platform_skill_dirs)
    singleton.send(:define_method, :platform_skill_dirs) { dirs }
    yield
  ensure
    singleton.send(:define_method, :platform_skill_dirs, original_method)
  end
