module DAG
  class Edge < ApplicationRecord
    ...
    validates :conversation_id, :from_node_id, :to_node_id, presence: true

    validate :nodes_belong_to_same_conversation
    ...
    private
      def nodes_belong_to_same_conversation
        ...
        if to_node && to_node.conversation_id != conversation_id
          errors.add(:to_node_id, "must belong to the same conversation")
        end
      end

      def must_not_introduce_cycle
        return if errors.any?
        return if conversation_id.blank? || from_node_id.blank? || to_node_id.blank?

        Conversation.lock.where(id: conversation_id).pick(:id)
