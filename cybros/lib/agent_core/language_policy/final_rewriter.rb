  ...
  module LanguagePolicy
    module FinalRewriter
        ...
        def rewrite(
          ...
          lang = AgentCore::LanguagePolicy::Detector.canonical_target_lang(target_lang)
          ValidationError.raise!(
            "target_lang is required",
            code: "agent_core.language_policy.final_rewriter.target_lang_is_required",
          ) if lang.empty?

          input = text.to_s
          return input if input.bytesize > DEFAULT_MAX_INPUT_BYTES
          if AgentCore::LanguagePolicy::Detector.language_shape(input, target_lang: lang) == :ok
            return input
          end
        ...
        def normalize_llm_options(value)
          h = value.nil? ? {} : value
          ValidationError.raise!(
            "llm_options must be a Hash",
            code: "agent_core.language_policy.final_rewriter.llm_options_must_be_a_hash",
          ...
          reserved = normalized.keys & AgentCore::OpenAI::RESERVED_CHAT_COMPLETIONS_KEYS
          if reserved.any?
            ValidationError.raise!(
              "llm_options contains reserved keys: #{reserved.map(&:to_s).sort.inspect}",
              code: "agent_core.language_policy.final_rewriter.llm_options_contains_reserved_keys",
