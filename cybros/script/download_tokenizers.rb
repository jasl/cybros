...
module DownloadTokenizers
  ...
  def run(argv)
    ...
    sources = Cybros::TokenEstimation.sources
    if options[:only].any?
      allowed = options[:only]
      sources = sources.select { |s| allowed.include?(s.fetch(:hint).to_s) }
    end
    ...
    tokenizer_root = options.fetch(:tokenizer_root)
    failures = []

    sources.each do |source|
      hint = source.fetch(:hint).to_s
      ...
      dest = Pathname.new(tokenizer_root).join(Cybros::TokenEstimation.tokenizer_relative_path(hint))
  ...
  def parse_options(argv)
    opts = {
      ...
      force: false,
      check: false,
      tokenizer_root: Cybros::TokenEstimation.tokenizer_root,
    }
    ...
    OptionParser.new do |parser|
      ...
      parser.on("--tokenizer-root PATH", "Tokenizer root (default: #{opts[:tokenizer_root]})") do |path|
        opts[:tokenizer_root] = path.to_s
      end
    ...
    opts[:only].delete("")
    opts[:tokenizer_root] = opts[:tokenizer_root].to_s.strip
    opts[:tokenizer_root] = Cybros::TokenEstimation.tokenizer_root if opts[:tokenizer_root].empty?
    opts
  end

  def hf_token
    token = ENV.fetch("HF_TOKEN", "").to_s.strip
    token = ENV.fetch("HUGGINGFACE_HUB_TOKEN", "").to_s.strip if token.empty?
  ...
  def source_tokenizer_family(source)
    return :hf_tokenizers if source.key?(:hf_repo) && !source.fetch(:hf_repo).to_s.strip.empty?

    source.fetch(:tokenizer_family, :tiktoken).to_s.strip.downcase.tr("-", "_").to_sym
