...
class AgentCoreTokenizationRegistryTest < ActiveSupport::TestCase
  test "register! validates hf_tokenizers require tokenizer_path" do
    err =
      assert_raises(AgentCore::ValidationError) do
        AgentCore::Tokenization::Registry.register!(
          {},
          hint: "deepseek-v3",
    ...
    assert_equal "agent_core.tokenization.registry.tokenizer_path_is_required_for_family", err.code
  end

  test "registry builds normalized entries" do
    registry =
      AgentCore::Tokenization::Registry.registry(
        sources: [
