...
class DAG::ProgrammableAgentSubagentFanoutTest < ActiveSupport::TestCase
  ...
  class OrchestratingAgentExecutor
    def execute(node:, context:, stream:)
      ...
      when "collect_subagents"
        ...
        DAG::ExecutionResult.finished(content: "done", usage: { "total_tokens" => 1 })
      end
    end
  ...
  test "parent-side orchestration fans out subagent runs and waits without using merge_lane_state" do
    ...
    begin
      ...
      run_tasks = graph.nodes.where(node_type: Messages::Task.node_type_key).select { |task| task.body_input["name"] == "subagent_run" }
      wait_tasks = graph.nodes.where(node_type: Messages::Task.node_type_key).select { |task| task.body_input["name"] == "subagent_wait" }
      task_names = graph.nodes.where(node_type: Messages::Task.node_type_key).map { |task| task.body_input["name"].to_s }

      assert_equal 2, run_tasks.length
      run_tasks.each do |task|
        tool_result = AgentCore::Resources::Tools::ToolResult.from_h(task.body_output.fetch("result"))
        assert_match(/\A[0-9a-f\-]{36}\z/, tool_result.metadata.dig("subagent", "subagent_id").to_s)
      end
      assert_equal 2, wait_tasks.length
      refute_includes task_names, "merge_lane_state"

      wait_tasks.each do |task|
        tool_result = AgentCore::Resources::Tools::ToolResult.from_h(task.body_output.fetch("result"))
        assert(
          tool_result.metadata.dig("subagent", "assistant_output_candidate").is_a?(Hash),
          "expected assistant_output_candidate in #{tool_result.metadata.inspect}",
        )
      end
