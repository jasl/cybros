class CreateDAGWorkflowEngine < ActiveRecord::Migration[8.2]
  def change
    ...
    create_table :dag_lanes, id: :uuid, default: -> { "uuidv7()" } do |t|
      ...
      t.bigint :next_lane_seq, null: false, default: 0
    ...
    create_table :dag_turns, id: :uuid, default: -> { "uuidv7()" } do |t|
      t.references :graph, null: false, type: :uuid,
                   foreign_key: { to_table: :dag_graphs, on_delete: :cascade }
      t.uuid :lane_id, null: false
      t.index %i[graph_id lane_id], where: "head_node_id_including_deleted IS NOT NULL",
              name: "index_dag_turns_graph_lane_visible_including_deleted"

      t.bigint :lane_seq
      t.check_constraint(
        "lane_seq IS NULL OR lane_seq > 0",
        name: "check_dag_turns_lane_seq_positive"
      )

      t.uuid :head_node_id
      t.datetime :head_created_at
      t.check_constraint(
        "((head_node_id IS NULL) = (head_created_at IS NULL))",
        name: "check_dag_turns_head_fields_consistent"
      )

      t.uuid :head_node_id_including_deleted
      t.datetime :head_created_at_including_deleted
      t.check_constraint(
        "((head_node_id_including_deleted IS NULL) = (head_created_at_including_deleted IS NULL))",
        name: "check_dag_turns_head_including_deleted_fields_consistent"
      )
      ...
      t.index %i[graph_id lane_id lane_seq],
              unique: true,
              where: "lane_seq IS NOT NULL",
              name: "index_dag_turns_graph_lane_lane_seq_unique"
