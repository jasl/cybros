...
- 可审计：关键操作与放权（权限、执行、网络）必须有 event 记录。
  - 包含 Cowork 的 Plan gate：plan 展示/确认/拒绝（可通过关联 DAG node_id 回放，无需在 event 中重复落内容）。
...
- `persona_id`（可选但建议：当 workflow 支持 persona switching 时记录；用于分析路由效果与用户体验差异）
- `agent_version`（优先 git commit SHA；否则 `config_version`；用于可回放/可归因）
- `prompt_builder_kind`（例如 `dsl` / `prompt_program`；用于分析“可编程 prompt building”的收益与风险）
- `prompt_builder_version`（优先 git commit SHA；否则 `config_version`；若发生降级/回退，仍应记录实际使用的版本）
- `prompt_build_latency_ms`（从开始构造 prompt 到得到可用 PromptSpec 的耗时）
- `prompt_build_error_code`（若发生失败并触发降级/回退：记录稳定错误码；不记录原始输出）
- `provider`
...
- 审计浏览：
  - 谁修改了系统配置
  - 谁批准了高风险执行
  - 谁确认/拒绝了 Cowork 的 Plan gate（用于排障与风险回溯）
