...
- settings 的作用域（global/space/user）
- secrets 的存储与审计原则
...
- settings 的作用域必须显式（默认 global），避免“看似只影响我，实际影响全局”的事故。
...
- 只支持 `global` scope（满足 self-hosted 运维）。
...
`user`（若允许） > `space`（若允许） > `global` > manifest default
...
- settings 修改是否需要提供 diff 与回滚（建议：至少 global settings 有审计与历史）？
