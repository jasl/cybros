...
- **App 层**：持久化“已批准规则”（conversation / user / global scope），并暴露为 prompt injection（让模型知道已批准哪些能力）
...
- ⏳ 未落地：app 层“已批准规则”的持久化与注入（conversation / user / global scope）
...
- ⏳ 未落地：`memory_get`、citations 契约、pre-compaction memory flush（auto_compact 前的 silent flush step）、分层写入策略（conversation/user/global scope 的 guardrails）
