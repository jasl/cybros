module RuntimeGovernance
  class ExecutionCapacityEnforcer
    ...
    def admit!(now:)
      capacity = execution_capacity_snapshot
      ...
      result.merge(execution_request_id: execution_request_id, capacity: capacity)
    end
      ...
      def execution_capacity_snapshot
        ...
        snapshot =
          if runtime_governors.is_a?(Hash)
            runtime_governors["execution_capacity"] || runtime_governors[:execution_capacity]
          end

        unless snapshot.is_a?(Hash)
          AgentCore::ValidationError.raise!(
            "Execution capacity snapshot missing",
            code: "cybros.runtime_governance.execution_capacity_snapshot_missing",
        ...
        AgentCore::ValidationError.raise!(
          "Execution capacity snapshot invalid",
          code: "cybros.runtime_governance.execution_capacity_snapshot_invalid",
          details: {
