...
module Agents
  class RPCClient
    ...
    RECOVERABLE_CONNECTION_ERRORS = [SocketError, EOFError, Errno::ECONNREFUSED, Errno::EHOSTUNREACH].freeze
    ...
    private
      ...
      attr_reader :agent, :recognized_deployment

      def deployment
        @deployment = @deployment.reload if @deployment.respond_to?(:reload)
        @deployment
      end
