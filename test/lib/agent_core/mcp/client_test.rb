...
class AgentCore::MCP::ClientTest < Minitest::Test
  ...
  def test_default_client_info
    prev = ENV["AGENT_CORE_CLIENT_VERSION"]
    ENV.delete("AGENT_CORE_CLIENT_VERSION")
    ...
    assert_equal "0", client_info["version"]
  ensure
    client&.close
    if prev
      ENV["AGENT_CORE_CLIENT_VERSION"] = prev
    else
      ENV.delete("AGENT_CORE_CLIENT_VERSION")
    end
  end

  def test_default_client_info_includes_version_from_env
    prev = ENV["AGENT_CORE_CLIENT_VERSION"]
    ENV["AGENT_CORE_CLIENT_VERSION"] = "test-1"
    ...
    assert_equal "agent_core", client_info["name"]
    assert_equal "test-1", client_info["version"]
  ensure
    client&.close
    if prev
      ENV["AGENT_CORE_CLIENT_VERSION"] = prev
