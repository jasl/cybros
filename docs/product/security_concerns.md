...
## 1) Execute 不可回滚（与 Auto-allow executes）

风险：

- `Execute` 可能造成不可逆后果（删文件、破坏 workspace、跑危险命令、污染依赖、资源消耗攻击）。
- 开启 `Auto-allow executes in this conversation` 会放大提示注入/误操作的影响面。
...
- **用户自助 Diagnostic Bundle**（导出诊断包）：
  - 默认仅包含元数据：版本号、error codes、events、turn_ids、tool/task 执行摘要、配置 commit shas、日志截断摘要等。
