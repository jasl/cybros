...
- **Resource scope（account/space/user）**：资源的归属边界（见 `docs/product/terminology.md`）。
- **Plugin install scope（system/space）**：插件安装范围；single-tenant 下 `system` = instance/account 级，`space` = 仅某个 Space 可见。
...
- 插件的 **设置**（system-level settings + admin UI）与可观测性。
...
- AgentProfile / PromptBuilder / Workflow 的模板（只读，可 Copy 到 Space repo 后再改）
...
我们只讨论 single-tenant，但仍然有 system 与 space 的边界：

- **System install**：全局可见；用于组织级标准化、官方/管理员维护的扩展。
...
- Setting value 的作用域必须显式：`system` / `space` / `user`（默认 `system`）。
- Setting 类型必须可验证：`bool/int/float/string/enum/json`（禁止随意对象反序列化）。
- secrets 不能出现在插件包内：
  - 插件只能声明 “需要一个 secret”，secret 的值由 admin 在 system settings 填入（并加密存储）。
  - 读取 secret 的能力必须在 capabilities 中显式出现，并落审计（至少记录“哪个插件读取了哪个 secret”，不记录 secret 值）。

备注：

- Phase 1 可以先只做 `system` 级 settings（instance/account-wide；满足 self-hosted 运维），再扩展到 `space/user`。
...
- 边界可解释：插件的安装范围（system/space install scope）与其产物资源的 resource scope（account/space/user）都必须在 UI 上可解释“为什么你能看到/不能看到”。
...
- 插件提供的是 **只读模板/能力**；用户的可变资源仍然由 git-backed Space repo 管理（见 `docs/product/versioning_and_sync.md`）。
- 插件更新不应“偷偷改写”用户 repo 的文件；除非用户明确执行 “Upgrade applied resources” 之类的动作，并且可回滚。

因此插件要提供两种内容交付方式（建议）：

1) **Template-only**：只在 UI 里可选择/可 Copy，不自动写入 repo。  
2) **Install-to-repo（可选）**：把模板写入某个 repo（产生 git commit + 审计），后续更新需要显式合并策略。
...
- Phase 1：Tier 0 Content plugins（System/Space 安装 + Copy 模板到 repo）
