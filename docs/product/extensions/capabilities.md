...
- `resources:install_to_repo`：允许一键写入 repo（必须产生 git commit + 审计）
...
- `secrets:read`：读取 system settings 中的 secret（必须配合 settings schema；审计必须记录“读取发生”，不记录值）
...
- 插件注册了工具 ≠ 工具默认可执行。
- 工具执行仍需遵守工具策略（默认 ConfirmAll）与 Runner policy（deny-by-default）。
